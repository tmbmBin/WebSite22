...
function ##ID##_loadUploaded() {
	...
	$.ajax({
		type: "POST", url: "##ROOT##/managex.ashx", dataType: "json", data: p,
		success: function (data) {
			...
			for (var i = 0; i < d.length; i++) {
				...
				var cancel = $(c+"<span class='fileName'>"
					+(dl=="1"? "<a href='##ROOT##/filex/?v=" + id.substring(0, 12) + "&s=" + s + "&k=" + id.substring(12) + "' class='"+s+ "' target='_blank'>":"")
...
function ##ID##_remove_click(e){
    ...
    $.ajax({
	    type: "POST", async: false, url: "##ROOT##/managex.ashx", data: pp,
	    success: function (data) {
...
function ##ID##_trashClear_click(){
	...
	if(confirm("清空全部檔案?\n\n"+"警告:此動作會將檔案徹底刪除且無法復原，是否確定刪除?")){
		...
		$.ajax({
			type: "POST", async: true, url: "##ROOT##/managex.ashx", data: p,
			success: function (d) {
				##ID##_loadUploaded();
			},
