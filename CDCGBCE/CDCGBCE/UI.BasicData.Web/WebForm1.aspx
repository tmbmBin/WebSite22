<%@ Page Language="vb" AutoEventWireup="false" CodeBehind="WebForm1.aspx.vb" Inherits="UI.BasicData.Web.WebForm1" %>
...
<head id="Head1" runat="server">
    ...
    <script language="javascript">
		...
		function fnGetInfo(){
			...
			{strValue= " And payCode = '" + document.all("TBpayCode").value +"'";}

			if (document.all("DDLPayKind").value !="")
			{		 
			    alert(" And payKind= '" + document.all("DDLPayKind").value + "'");
				//strValue += " And payKind= '" + document.all("DDLPayKind").value + "'" ;
				 //alert(strValue);
			}

			if (document.all("TBpayID").value !="")
			{			 		
				strValue += " And payID='"+ document.all("TBpayID").value +"'";
			}


			if (document.all("TBpayName").value !="")
			{			 		
				strValue += " And payName Like '%" + document.all("TBpayName").value +"%'";					               
			}

			if (document.all("TBpayAliasNo").value !="")			
			{strValue += " And payAliasNo Like '%" + document.all("TBpayAliasNo").value +"%'";}
...
<body>
 ...
 <table style="table-layout:fixed" border=0 cellpadding=0 cellspacing=0 width=100%>
                                   ...
                                   <tr >
                                      <td style="WIDTH: 120px; padding-right:2px" align="right" ><INPUT id="BtnOK" type="button" value="確認" name="BtnOK" runat="server" Width="60px" style="WIDTH: 60px; HEIGHT: 22px"
