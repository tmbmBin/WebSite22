<Script Language="VB" Runat="Server">
	...
	Function MySqlDbWriter(queryString As String, _
					Optional myConnection As System.Data.SqlClient.SqlConnection = Nothing, _
					Optional myCommand As System.Data.SqlClient.SqlCommand = Nothing, _
					Optional NoTry As Boolean = False)
		Dim dbConnection As System.Data.IDbConnection = IIf(myConnection Is Nothing, New System.Data.SqlClient.SqlConnection(connectionString), myConnection)
		Dim dbCommand As System.Data.SqlClient.SqlCommand = IIf(myCommand Is Nothing, New System.Data.SqlClient.SqlCommand(queryString, dbConnection), myCommand)
...
'}
'並於 Page_Load() 中使用
'v2+: DisableAfterSubmit("ctl00") 或 v1.x: DisableAfterSubmit("_ctl0")
...
'====================================================================================================
	...
	'設定 DR("日期") 至 TextBox
	Sub set_TBoxCDate(sTBOX As Object, sVAL As Object)
		If IsDate(sVAL & "") = True Then
			sTBOX.Text = ChtDate(sVAL)
		Else
			sTBOX.Text = "　/　/　"
		End If
	End Sub
	...
	'取得 TextBox 日期
	Function get_TBoxCDate(ByRef sTBOX As Object, Optional onError As Object = "　/　/　") As Object
		If IsDate(EngDate(sTBOX.Text)) = True Then
			Return EngDate(sTBOX.Text)
		Else
			sTBOX.Text = onError
			Return Nothing
		End If
	...
	'設定 DR("金額") 至 TextBox
	Sub set_TBoxMoney(sTBOX As Object, sVAL As Object, Optional vFORMAT As String = "#,0", Optional onError As Object = "")
		Try
			sTBOX.Text = Format(CDbl(Val(sVAL & "")), vFORMAT)
		Catch
			sTBOX.Text = onError
		End Try
	End Sub
	...
	'取得 TextBox 金額
	Function get_TBoxMoney(ByRef sTBOX As Object, Optional onError As Object = "0") As Object
		If IsNumeric(Replace(Trim(sTBOX.Text), ",", "")) = True Then
			Return Val(Replace(Trim(sTBOX.Text), ",", ""))
		Else
			sTBOX.Text = onError
			Return Nothing
		End If
